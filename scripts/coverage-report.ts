...
const REPO_URL = 'https://github.com/petarzarkov/arkv';
...
const packageSection = (pkg: PackageCoverage): string => {
  ...
  <summary>
    <span class="pkg">@arkv/${pkg.name}</span>
...
${
  untested.length
    ? `<p class="untested">No test files found in: ${untested.map((n) => `<code>@arkv/${n}</code>`).join(', ')}</p>`
    : ''
}
<script>
